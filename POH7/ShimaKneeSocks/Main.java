...
public class Main {
    ...
    void solve() throws Exception {
        ...
        for (int i = n; i-- > 0;) sb.append('R');
        for (int i = n; i-- > 0;) sb.append('W');
        
        while (sb.length() < m) {
            sb.append(sb.toString());
        }
        
        println(sb.substring(0, m));
    }
    ...
    String[]   scnss() { return null; }
    int[]      scnis() { String[] ss = scnss(); int[] r = new int[ss.length]; int i = 0; for (String s : ss) r[i++] = ti(s); return r; }
    ...
    public static void main(String[] args) throws Exception {
        Main m = new Main(){
            ...
            @Override String[] scnss() { next(); buf = Arrays.copyOfRange(buf, cur, buf.length); cur = buf.length; return buf; }
            @Override String   scnln() { if (buf == null || buf.length == cur) return gs(); else return String.join(" ", scnss()); }
        }; m.init(); m.solve();
    }
    // ==================================================================================
}
