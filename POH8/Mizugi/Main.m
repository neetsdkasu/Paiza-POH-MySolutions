...
void solve(MyIO *io) {
    ...
    for (NSUInteger i = 0; i < n; i++) {
        id key = [s substringWithRange: NSMakeRange(i, 1)];
        id val = [d objectForKey: key];
        if (val) {
            [d setObject: wrap([val integerValue] + 1) forKey: key];
        } else {
            [d setObject: @1 forKey: key];
        }
    }
    ...
    for (NSUInteger i = 0; i < m; i++) {
        id key = [t substringWithRange: NSMakeRange(i, 1)];
        id val = [d objectForKey: key];
        if (val) {
            if ([val isEqual: @0]) {
                answer++;
